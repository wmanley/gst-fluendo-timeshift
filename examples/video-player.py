...
class Player(object):
    def __init__(self, src_description):
        ...
        self.playbin = Gst.parse_bin_from_description(
            src_description + \
            ' ! queue ! tsparse' \
            ' ! flumpegshifterbin name=timeshifter recording-template=/tmp/ts.XXXXXX' \
            ' ! tsdemux ! video/x-h264 ! decodebin ! queue ! autovideosink',
            False);
...
def main(argv):
    if len(sys.argv) == 1:
        ...
        src = 'filesrc location=%s' % (sys.argv[1])
    p = Player(src)
    p.run()
